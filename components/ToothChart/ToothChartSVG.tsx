// components/ToothChart/ToothChartSVG.tsx
"use client";
  ...
  const updateToothAppearance = useCallback(() => {
    teethData.forEach((tooth) => {
      ...
      if (toothElement) {
        ...
        if (isActive) {
          // حالة نشطة (محدد أو هوفر): شفافية كاملة + حدود سوداء
          toothElement.setAttribute("opacity", "1");
          toothElement.setAttribute("fill-opacity", "1");
          toothElement.setAttribute("stroke-opacity", "1");
          toothElement.setAttribute("stroke", "#4B5563"); // أسود غامق
          toothElement.setAttribute("stroke-width", isSelected ? "2.5" : "1.5");
          toothElement.setAttribute(
            "filter",
            isSelected ? "url(#highlight)" : "url(#hover-shadow)",
          );
        } else {
          // الحالة الافتراضية: تعتمد على وجود سن محدد
          // إذا لم يكن هناك سن محدد: شفافية 0.7
          // إذا كان هناك سن محدد: شفافية 0.4
          ...
          toothElement.setAttribute("stroke", "#C0C0C0"); // فضي
      ...
      if (txtElement) {
        ...
        if (isActive) {
          ...
          // نفس منطق الشفافية للنصوص غير النشطة
          const txtOpacity = selectedToothId === null ? "0.7" : "0.5";
          txtElement.setAttribute("opacity", txtOpacity);
        }
      }
    });
...
const updateLabelsAndOutlines = () => {
  ...
  // تطبيق تأثير الهوفر على سن معين
  const applyHoverEffect = (toothNumber: number | null) => {
    // إزالة تأثير الهوفر من السن السابق
    if (hoveredToothRef.current !== null) {
      const prevElement = document.getElementById(
        `Tooth${hoveredToothRef.current}`,
      );
      if (prevElement && hoveredToothRef.current !== selectedToothId) {
        // إعادة السن السابق إلى الحالة الافتراضية
        // تعتمد الشفافية على وجود سن محدد من عدمه
        const baseOpacity = selectedToothId === null ? "0.7" : "0.4";
        ...
        // إعادة النص المرتبط إلى الشفافية المناسبة
        const prevTxtElement = document.getElementById(
          `txtTooth${hoveredToothRef.current}`,
        );
        if (prevTxtElement) {
          const txtOpacity = selectedToothId === null ? "0.7" : "0.5";
          prevTxtElement.setAttribute("opacity", txtOpacity);
        }
      }
    }

    // تطبيق تأثير الهوفر على السن الجديد
    if (toothNumber !== null && toothNumber !== selectedToothId) {
      ...
      if (element) {
        ...
        // جعل النص المرتبط كامل الشفافية
        const txtElement = document.getElementById(`txtTooth${toothNumber}`);
        if (txtElement) {
          const toothData = teethData.find((t) => t.id === toothNumber);
          const procedureText =
            toothData?.customProcedure ||
            (toothData?.procedure !== "sound"
              ? getProcedureLabel(toothData?.procedure || "")
              : "");
  ...
  // تهيئة الحالة الافتراضية عند التحميل
  useEffect(() => {
    const initializeOpacity = () => {
      // شفافية افتراضية: 0.7 لأن selectedToothId سيكون null في البداية
      ...
      teethData.forEach((tooth) => {
        ...
        const txtElement = document.getElementById(`txtTooth${tooth.id}`);
        if (txtElement) {
          txtElement.setAttribute("opacity", "0.7");
        }
      });
    ...
    const timer = setTimeout(initializeOpacity, 100);
    ...
    const handleMouseOut = (e: MouseEvent) => {
      ...
      if (toothId.startsWith("Tooth") && target.closest("#Spots")) {
        ...
        if (!isNaN(toothNumber) && toothNumber !== selectedToothId) {
          // إعادة السن إلى الحالة الافتراضية
          const element = document.getElementById(`Tooth${toothNumber}`);
          if (element) {
            // تعتمد الشفافية على وجود سن محدد من عدمه
            const baseOpacity = selectedToothId === null ? "0.7" : "0.4";
            ...
            // إعادة النص المرتبط
            const txtElement = document.getElementById(
              `txtTooth${toothNumber}`,
            );
            if (txtElement) {
              const toothData = teethData.find((t) => t.id === toothNumber);
              txtElement.setAttribute(
                "fill",
                toothData?.procedure === "sound" ? "#9CA3AF" : "#374151",
              );
              txtElement.setAttribute("font-size", "10px");
              txtElement.setAttribute("font-weight", "normal");
              // نفس منطق الشفافية للنصوص
              const txtOpacity = selectedToothId === null ? "0.7" : "0.5";
              txtElement.setAttribute("opacity", txtOpacity);
            }
          }
    ...
    <div ref={svgRef} className="tooth-chart-svg-container">
      <svg
        ...
        style={{
          ...
          display: "block",
          margin: "0 auto",
          backgroundColor: "#f8fafc", // خلفية فاتحة لإظهار الشفافية
        }}
      >
        <defs>
          {/* فلتر التمييز عند الاختيار - توهج واضح */}
