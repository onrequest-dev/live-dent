      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6">
        ...
        <div className="relative z-10 text-center mb-6 md:mb-10">
          ...
          >
            ...
            <span className="text-yellow-400 text-xs md:text-sm font-medium">{subtitle}</span>
          </motion.div>
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="hidden md:block relative rounded-3xl overflow-hidden border-2 border-yellow-500/20 bg-[#0A1628] shadow-2xl shadow-yellow-500/5" style={{ perspective: "1200px" }}>
            <div className="relative aspect-[16/10] lg:aspect-[16/9] overflow-hidden group">
              ...
              {/* أزرار التنقل */}
              <button onClick={goPrev} className="absolute left-3 top-1/3 w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-black/30 hover:bg-black/50 backdrop-blur-sm border border-white/20 hover:border-yellow-400/50 flex items-center justify-center transition-all duration-300 opacity-0 group-hover:opacity-100 z-20">
                <ChevronLeft className="w-5 h-5 lg:w-6 lg:h-6 text-white" />
              </button>
              <button onClick={goNext} className="absolute right-3 top-1/3 w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-black/30 hover:bg-black/50 backdrop-blur-sm border border-white/20 hover:border-yellow-400/50 flex items-center justify-center transition-all duration-300 opacity-0 group-hover:opacity-100 z-20">
              ...
              <div className="absolute top-4 left-4 right-4 z-20">
                <div className="flex items-center justify-between">
                  <div className="bg-black/40 backdrop-blur-sm rounded-full px-4 py-1.5 border border-white/10">
                    <h3 className="text-white font-bold text-sm lg:text-base">{activeImage.title}</h3>
                  </div>
                  <div className="bg-black/40 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/10">
                    <span className="text-yellow-400 text-xs font-medium">{activeIndex + 1} / {images.length}</span>
                  </div>
                </div>
              </div>

              {/* صور مصغرة 3D للمكتبي */}
              <div className="absolute bottom-0 left-0 right-0 z-20 pb-4 px-6" style={{ perspective: "800px" }}>
                    ...
                    return (
                      <motion.div
                        ...
                        animate={{
                          ...
                          filter: isActive ? 'brightness(1.1) saturate(1.2)' : `brightness(${0.6 - distance * 0.08})`,
                        }}
                        transition={{ type: "spring", stiffness: 200, damping: 25 }}
                        whileHover={{ scale: isActive ? 1.35 : 0.95, y: isActive ? -12 : -2, rotateX: 30 }}
                      ...
                      >
                        <div className={cn(
                          ...
                          isActive ? "ring-2 ring-yellow-400 ring-offset-2 ring-offset-[#0A1628] shadow-xl shadow-yellow-500/40" : "ring-1 ring-white/20 hover:ring-yellow-500/40"
                        )}>
                          <img src={image.src} alt={image.alt} className="w-full h-full object-cover" loading="lazy" />
                          {!isActive && <div className="absolute inset-0 bg-black/30 backdrop-blur-[1px]" />}
                          {isActive && (
                            <>
                          ...
                          {isActive && isAutoPlaying && (
                            <motion.div initial={{ scaleX: 1 }} animate={{ scaleX: 0 }} transition={{ duration: 5, ease: "linear" }} className="absolute top-0 left-0 right-0 h-0.5 bg-yellow-400 origin-left" />
                          )}
                        </div>
                      </motion.div>
                    )
            ...
            <div className="relative px-6 py-4 border-t border-yellow-500/10 bg-[#0A1628]/80 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <p className="text-gray-400 text-sm">{activeImage.description}</p>
                <div className="flex items-center gap-1.5">
                  {images.map((_, index) => (
                    <button key={index} onClick={() => goToImage(index)} className={cn("rounded-full transition-all duration-300", index === activeIndex ? "w-5 h-1.5 bg-yellow-400" : "w-1.5 h-1.5 bg-yellow-500/30 hover:bg-yellow-500/50")} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="md:hidden">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-yellow-500/20 bg-[#0A1628] shadow-xl mb-4">
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                {/* أزرار التنقل */}
                <button onClick={goPrev} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 flex items-center justify-center z-20 active:scale-90 transition-transform">
                  <ChevronLeft className="w-4 h-4 text-white" />
                </button>
                <button onClick={goNext} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 flex items-center justify-center z-20 active:scale-90 transition-transform">
                ...
                <div className="absolute top-3 left-3 right-3 z-20">
                  <div className="flex items-center justify-between">
                    <div className="bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 border border-white/10">
                      <h3 className="text-white font-bold text-xs">{activeImage.title}</h3>
                    </div>
                    <div className="bg-black/50 backdrop-blur-sm rounded-full px-2 py-1 border border-white/10">
                      <span className="text-yellow-400 text-[10px] font-medium">{activeIndex + 1}/{images.length}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="px-3 py-2 border-t border-yellow-500/10 bg-[#0A1628]">
                <p className="text-gray-400 text-xs">{activeImage.description}</p>
              </div>
            </div>
      ...
      return (
        <motion.button
          ...
          animate={{
            ...
            opacity: isActive ? 1 : Math.max(0.4, 0.8 - distance * 0.15),
            scale: isActive ? 1.1 : 1,
            marginLeft: distance === 1 && !isActive ? '-8px' : '0px',
            marginRight: distance === 1 && !isActive ? '-8px' : '0px',
